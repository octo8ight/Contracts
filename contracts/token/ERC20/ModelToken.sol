...
contract ModelToken is Context, IERC20, IERC20Metadata {
    ...
    function remove_stake(
        uint256 amount
    ) public returns (uint256 removed_amount) {
    ...
    function set_alpha(uint256 alpha) public {
        ...
        require(
            alpha < PERCENT_BASE,
            "ALPHA needs to be Lower than Percent Base"
        );
        ALPHA = alpha;
    }
    ...
    function set_votes(uint256[] memory votes, address[] memory devs) public {
        require(
            votes.length == devs.length,
            "votes and devs have to be same length bro"
        );
        require(
            votes.length < PERCENT_BASE,
            "the size of you voters is too big fam"
        );
        require(
            dev2state[msg.sender].stake > 0,
            "you need to stake greater than 0"
        );

        uint256 total_score = 0;
        uint256 current_score;
        uint256[] memory previous_normalized_scores = new uint256[](
            votes.length
        );
        ...
        for (uint i; i < devs.length; i++) {
            previous_normalized_scores[i] = voter2dev_vote_map[voter][devs[i]]
                .score;
        ...
        for (uint d_i; d_i < voter2devs[voter].length; d_i++) {
            ...
            for (uint v_i; v_i < num_voters_for_dev; v_i++) {
                ...
                if (voter == dev2voters[dev][v_i]) {
                    ...
                    dev2voters[dev][v_i] = dev2voters[dev][
                        dev2voters[dev].length - 1
                    ];
        ...
        for (uint i; i < votes.length; i++) {
            ...
            voter2dev_vote_map[voter][dev] = VoteState({
        ...
        for (uint i; i < votes.length; i++) {
            dev = devs[i];
            _ensure_dev(dev);
            voter2dev_vote_map[voter][dev].score =
                (voter2dev_vote_map[voter][dev].score * PERCENT_BASE) /
                (total_score);
            voter2dev_vote_map[voter][dev].score = ((previous_normalized_scores[
                i
            ] *
                ALPHA +
                voter2dev_vote_map[voter][dev].score *
                (PERCENT_BASE - ALPHA)) / PERCENT_BASE);
